<?php
/**
 * This file handles AffiliateManagerForm authentication via sms notification
 *
...
class AffiliateManagerForm extends FormInterface {
	 ...
	 * @return void
	 */
	public function sa_update_billing_phone( $user_id ) {
	...
	public function routeData() {
		...
		switch ( trim( sanitize_text_field( wp_unslash( $_REQUEST['handler'] ) ) ) ) {
			...
			case 'deactivateAffiliate':
				$this->after_changed_wpam_status( $_REQUEST );
				break;
			case 'addTransaction':
	 ...
	 * @return void
	 */
	public static function after_changed_wpam_status( $data = array() ) {
		$affiliate_id     = $data['affiliateId'];
		$am_user          = self::getAffiliateById( $affiliate_id );
		$status           = $data['handler'];
		$buyer_sms_notify = smsalert_get_option( 'wpam_order_status_' . $status, 'smsalert_wpam_general', 'on' );
	...
	public static function isFormEnabled() {
		return is_plugin_active( 'affiliates-manager/boot-strap.php' ) ? true : false;
	}
