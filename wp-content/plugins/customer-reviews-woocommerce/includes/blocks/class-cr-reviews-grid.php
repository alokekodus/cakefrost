	...
	*/
	final class CR_Reviews_Grid {
		...
		*/
		public function render_reviews_grid( $attributes ) {
			...
			if( 'initials' === $attributes['avatars'] ) {
				add_filter( 'get_avatar', array( $this, 'cr_get_avatar' ), 10, 5 );
			}
			ob_start();
			include( $template );
			if( 'initials' === $attributes['avatars'] ) {
				remove_filter( 'get_avatar', array( $this, 'cr_get_avatar' ) );
			}
		...
		*/
		...
		public function cr_get_avatar( $avatar, $id_or_email, $size = 96, $default = '', $alt = '' ){
			if ( is_object( $id_or_email ) && isset( $id_or_email->comment_ID ) ) {
				...
				if( 0 < mb_strlen( $author ) ) {
					...
					$avatar = sprintf( '<img alt="%s" src="%s">', $alt, 'data:image/svg+xml;base64,' . base64_encode( $svg ) );
				}
			}
			return $avatar;
		}
