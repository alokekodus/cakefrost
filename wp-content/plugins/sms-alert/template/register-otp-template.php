...
if (! SmsAlertUtility::isBlank($user_email) || ! SmsAlertUtility::isBlank($phone_number) ) {
    echo '								<div class="sa_customer_validation-login-container">
												<form name="f" method="post" action="">
													...
													<input type="number" name="smsalert_customer_validation_otp_token"  autofocus="true" placeholder="" id="smsalert_customer_validation_otp_token" required="true" class="sa_customer_validation-textbox" autofocus="true" pattern="[0-9]{4,8}" title="' . esc_attr(SmsAlertMessages::showMessage('OTP_RANGE')) . '" />
...
if (! $from_both ) {
    ...
    echo '				<input type="hidden" id="from_both" name="from_both" value="true" />' . PHP_EOL;
        ...
        echo '		</form>
					...
					input[type="number"].sa_customer_validation-textbox {background: #FBFBFB none repeat scroll 0% 0%;font-family: "Open Sans",sans-serif;font-size: 24px;width: 100%;border: 1px solid #DDD;padding: 3px;margin: 2px 6px 16px 0px;}
					</style>
					<script>
