...
class WpMemberForm extends FormInterface {
	...
	public function add_shortcode_to_wpmember( $sa_type, $field ) {
		...
		$field_content .= '<script>jQuery(document).ready(function(){$sa=jQuery;$sa("#smsalert_otp_token_submit").click(function(o){ ';
		$field_content .= 'var e=$sa("input[name=' . $field . ']").val(); $sa("#salert_message").empty(),$sa("#salert_message").append("Sending OTP..."),';
		$field_content .= '$sa("#salert_message").show(),$mo.ajax({url:"' . site_url() . '/?option=smsalert-wpmember-form",type:"POST",';
