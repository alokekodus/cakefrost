...
class SAShipmentIntegration
{
    ...
    public static function get_ast_addon_templates()
    {
        ...
        $text_body = smsalert_get_option('ast_notify', 'smsalert_ast_message', sprintf(/* translators: %1$s: Item name */esc_html_e('Shipped: %1$s has been dispatched via %2$s with tracking number %3$s. Track here %4$s', 'sms-alert'), '[item_name_qty]', '[tracking_provider]', '[tracking_number]', '[tracking_link]'));
...
class SAWCAuctions
{
    ...
    public static function get_customer_templates()
    {
        ...
        $templates['outbid']['text-body']      = smsalert_get_option('wcauction_sms_body_outbid', 'smsalert_wcauction_message', sprintf(/* translators: %1$s: First name */esc_html_e('Hello %1$s, a new bid for auction %2$s has just been submitted. The new bid is: %3$s. Please visit the auction %4$s', 'sms-alert'), '[first_name]', '[auction_name]', '[auction_bid]', '[auction_link]'));
        ...
        $templates['customerbid']['text-body']      = smsalert_get_option('wcauction_sms_body_customerbid', 'smsalert_wcauction_message', sprintf(/* translators: %1$s: First name */esc_html_e('Hello %1$s, Thank You for placing bid for %2$s. Your bid is %3$s. Please visit the auction %4$s', 'sms-alert'), '[first_name]', '[auction_name]', '[auction_bid]', '[auction_link]'));
    ...
    public static function get_admin_templates()
    {
        ...
        $text_body = smsalert_get_option('wcauction_admin_sms_body_new', 'smsalert_wcauction_message', sprintf(/* translators: %1$s: Store name */esc_html_e('%1$s a new bid for auction %2$s has been submitted by %3$s. The new bid is: %4$s. Please visit the auction %5$s', 'sms-alert'), '[store_name]:', '[auction_name]', '[first_name]', '[auction_bid]', '[auction_link]'));
